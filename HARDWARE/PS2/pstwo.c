...
void PS2_Init(void)
{
  GPIO_InitTypeDef GPIO_InitStructure;
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3;	        //Port configuration //端口配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;  //Pull the output //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; //50M
	GPIO_Init(GPIOD, &GPIO_InitStructure);					  //Initialize the gpioD according to the set parameter //根据设定参数初始化GPIOD

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_15;	  //Port configuration //端口配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPD; //Drop input //下拉输入
	GPIO_Init(GPIOE, &GPIO_InitStructure);				//Initialize the gpiodE according to the set parameter //根据设定参数初始化GPIOE

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12|GPIO_Pin_13; //Port configuration //端口配置
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;       //Pull the output //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;      //50M
	GPIO_Init(GPIOB, &GPIO_InitStructure);					       //Initialize the gpiodB according to the set parameter //根据设定参数初始化GPIOB
}
/**************************************************************************
